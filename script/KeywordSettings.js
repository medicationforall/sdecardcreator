...
this.setupList = function(selected){
	...
	for(var i=0,key;key=keywordStore.ordered[i];i++){
...
this._setupSaveClick = function(){
	...
	$('.keyword.settings .kSave').click(function(event){
		...
		if(name!==''){
			...
			var data = that.gatherData();

			//set the data object to the store
			//console.log('clicked kSave',data);
			keywordStore.setCustomKey(data);
...
this.gatherData = function(){
	...
	var exists = keywordStore.data[data.name]!==undefined;

	//correctly resolve 
	if(exists){
		data.description= keywordStore.data[data.name].description;
	}else{
		data.description="";
	}
	...
	if(count>1){
		...
		$('.keyword.settings select[name="eVersion"] option').each(function(index,node){
			if(skip){
				...
				if(exists && keywordStore.data[data.name].errata[index-1]){
					errata.description =  keywordStore.data[data.name].errata[index-1].description;
				}else{
					errata.description="";
				}
...
this.fillOutForm = function(key){
	var data = keywordStore.data[key];
