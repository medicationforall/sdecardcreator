...
this._getData = function(){
	var that = this;
	$.getJSON('https://sde.medicationforall.com/keywordlist.php?json=true',function(data){
		//adds the data to a global scope object
		that._setup(data);
...
this._setup= function(data){
	...
	//console.log('sorted ',this.ordered);
...
this.findKeywords =  function(text){
	...
	text = text.replace(this.re,function(match){
		var key = that.lookup[match.toLowerCase()];
		var keyClass = that.resolveKeyClass(key);
...
this.findNKeywords = function(text){
	...
	text = text.replace(this.reN,function(match,key,number){
		...
		var dataKey = that.resolveNKey(key);
		var keyClass = that.resolveKeyClass(key);
...
this.setupKeywordsForm = function(){
	...
	$('.form input[name=keywordsList]').autocomplete({
		...
		source: function( request, response ) {
			...
			response( $.ui.autocomplete.filter(that.ordered, that.extractLast( request.term ) ) );
		},
		focus: function() {
...
this.checkKeywords = function(node){
	...
	$(node).find('.keyword').each(function(index, element){
		//console.log('found keyword for back',$(element).data('key'));
	...
	for(var i=0,key;key=keysFound[i];i++){
		that.addKeyword(key,that.data[key]);
	}
}
